...
// Calcula o multiplicador de tipo (Versão "Nuclear" - Compara tudo em minúsculo)
function getTypeEffectiveness(moveType, defenderTypes, typeData) {
    ...
    try {
        ...
        // 1. Achar o tipo do ATACANTE (ex: Planta)
        ...
        // 2. Comparar com os tipos do DEFENSOR (ex: Água)
        defenderTypes.forEach(defType => {
            ...
            // O SEGREDO: Converte o alvo para minúsculo (ex: "água")
            const targetToCheck = dTypePT.toLowerCase(); 
            
            // Verifica Fraquezas (Super Efetivo - 1.6x)
            if (typeInfo.ataque.dobro) {
                // Transforma a lista do banco em minúsculo antes de verificar
                // Isso garante que ["Água"] vire ["água"] e bata com o targetToCheck
