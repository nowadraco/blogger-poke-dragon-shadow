...
// 📁 PASTA 8: O FUNIL DE 3 ESTÁGIOS (M6 -> M7 -> M8)
const pastaDestino = path.join(__dirname, 'json', 'simulacao_pve8');
...
// ====================================================================
// 📊 FUNÇÃO AJUDANTE: RODA MÚLTIPLAS LUTAS DE RNG
// ====================================================================
function rodarMonteCarlo(simularFn, qtdLutas) {
    ...
    for(let i=0; i < qtdLutas; i++){
        let r = simularFn(1, true); // true = RNG ligado
...
// 4. FUNÇÃO PRINCIPAL (O TRIPLO FUNIL: M6 -> M7 -> M8)
async function gerarRankingEmMassa(bossesInput, tiersInput) {
    ...
    for (let i = 0; i < listaBosses.length; i++) {
        ...
        for (const currentTier of tiersToRun) {
            ...
            console.log(`\n========================================================`);
            console.log(`💎 MOTOR 8.0 (TRIPLO FUNIL: M6 -> M7 -> M8): ${bossData.speciesName.toUpperCase()}`);
            console.log(`========================================================`);
            ...
            for (let c = 0; c < cenariosDeLuta.length; c++) {
                ...
                // =======================================================
                // 🚪 ESTÁGIO 1: MOTOR 6.0 (Determinístico -> Peneira 500)
                // =======================================================
                let resultadosM6 = [];
                let c1 = 0;
                ...
                atacantesVIP.forEach(atacante => {
                    c1++;
                    if (c1 % 10 === 0 || c1 === atacantesVIP.length) desenharBarra(c1, atacantesVIP.length, '🔍', `Fase 1 (M6)`);
                ...
                // Filtro do Motor 6 (Aprovando os Top 500)
                let aprovadosM6 = new Map();
                const add500 = (lista) => lista.slice(0, 500).forEach(combo => aprovadosM6.set(combo.hash, combo));
                add500([...resultadosM6].sort((a, b) => a.eMedio - b.eMedio)); 
                add500([...resultadosM6].sort((a, b) => b.er - a.er));         
                add500([...resultadosM6].sort((a, b) => b.dMax - a.dMax));     
                add500([...resultadosM6].sort((a, b) => b.dp - a.dp));         
                add500([...resultadosM6].sort((a, b) => b.tdoMax - a.tdoMax)); 
                add500([...resultadosM6].sort((a, b) => a.ttw - b.ttw));       
                add500([...resultadosM6].sort((a, b) => a.mMin - b.mMin));     

                const listaM7 = Array.from(aprovadosM6.values());

                // =======================================================
                // 🎲 ESTÁGIO 2: MOTOR 7.0 (RNG 20x -> Peneira 250)
                // =======================================================
                let resultadosM7 = [];
                let c2 = 0;
                ...
                listaM7.forEach(combo => {
                    c2++;
                    if (c2 % 5 === 0 || c2 === listaM7.length) desenharBarra(c2, listaM7.length, '🎲', `Fase 2 (M7)`);
                    ...
                    if(simular) {
                        const m7Data = rodarMonteCarlo(simular, 20); // 20x
                ...
                // Filtro do Motor 7 (Aprovando os Top 250 provados pelo RNG 20x)
                let aprovadosM7 = new Map();
                const add250 = (lista) => lista.slice(0, 250).forEach(combo => aprovadosM7.set(combo.hash, combo));
                add250([...resultadosM7].sort((a, b) => a.e - b.e));   // e = estimador médio do RNG
                add250([...resultadosM7].sort((a, b) => b.er - a.er)); // er = score
                add250([...resultadosM7].sort((a, b) => b.d1 - a.d1)); // d1 = DPS Máximo na sorte
                add250([...resultadosM7].sort((a, b) => b.dp - a.dp)); // dp = dano %
                add250([...resultadosM7].sort((a, b) => b.td1 - a.td1)); // td1 = TDO Máximo
                add250([...resultadosM7].sort((a, b) => a.tw - b.tw)); // tw = TTW médio
                add250([...resultadosM7].sort((a, b) => a.m0 - b.m0)); // m0 = Mínimo de Mortes

                const listaM8 = Array.from(aprovadosM7.values());

                // =======================================================
                // 💎 ESTÁGIO 3: MOTOR 8.0 (RNG 100x na Nata da Nata)
                // =======================================================
                ...
                listaM8.forEach(combo => {
                    c3++;
                    if (c3 % 5 === 0 || c3 === listaM8.length) desenharBarra(c3, listaM8.length, '💎', `Fase 3 (M8)`);

                    const atacanteObj = atacantesVIP.find(p => p.speciesId === combo.id);
                    const simular = instanciarCombate(atacanteObj, oponenteRaid, combo.f, combo.c, configRaid.tempo);
                    if(simular) {
                        const m8Data = rodarMonteCarlo(simular, 100); // 100x de Profundidade Absoluta!
                        resultadosFinais.push({
                            i: atacanteObj.speciesId, n: atacanteObj.speciesName, f: combo.f.replace("_FAST", ""), c: combo.c, ...m8Data
                        });
                    }
                });
                console.log();
            ...
            console.log(`\n✅ ARQUIVO 8.0 GERADO: ${nomeDoArquivo} (${tamanhoKB} KB)`);
        }
    }
}
...
console.log("====================================================");
console.log("💎 GERADOR PVE 8.0 (TRIPLO FUNIL DE ALTA PRECISÃO) 💎");
console.log("====================================================\n");
