    ...
    /*
    =================================================
      SCRIPT DE REIDES COM DETALHES
    =================================================
    */

    function limparNomePokemonDetalhes(nome) {
        if (!nome) return '';
        return nome
    ...
    // ▼▼▼ URLS CORRIGIDAS ▼▼▼
    const URL_DETALHES_POKEMONS = 'https://cdn.jsdelivr.net/gh/nowadraco/blogger-poke-dragon-shadow@main/json/poke_reide.json';
    const URL_DETALHES_SHINIES = 'https://cdn.jsdelivr.net/gh/nowadraco/blogger-poke-dragon-shadow@main/json/poke_reide_shiny.json';
    const URL_DETALHES_TIPOS = 'https://cdn.jsdelivr.net/gh/nowadraco/bloggerpoke@main/src/data/gamemaster/tipos_poke.json';
    // ▲▲▲ URLS CORRIGIDAS ▲▲▲

    async function carregarDadosDetalhes() {
        try {
            const [resPokemons, resShinies, resTipos] = await Promise.all([
                fetch(URL_DETALHES_POKEMONS),
                fetch(URL_DETALHES_SHINIES),
                fetch(URL_DETALHES_TIPOS)
            ]);
            const pokemons = await resPokemons.json();
            const shinyPokemons = await resShinies.json();
            const dadosDosTipos = await resTipos.json();
            return { pokemons, shinyPokemons, dadosDosTipos };
        } catch (error) {
            console.error('❌ Erro ao carregar os arquivos JSON para Detalhes:', error);
            return { pokemons: [], shinyPokemons: [], dadosDosTipos: [] };
        }
    }
    ...
    function formatarTabelaTiposDetalhes(dadosDefensivos) {
        ...
        const todosOsNomesDeTipos = dadosDefensivos.map(t => t.tipo);
        todosOsNomesDeTipos.forEach(nomeTipo => {
            tabelaOfensiva[nomeTipo] = { ataca: {} };
        });
    ...
    function calcularFraquezasDetalhes(tiposDoPokemon, tabelaDeTipos) {
        ...
        const todosOsTiposAtacantes = Object.keys(tabelaDeTipos);
        todosOsTiposAtacantes.forEach(tipoAtacante => {
    ...
    function generatePokemonListItemDetalhes(pokemon, shinyPokemon, nomeExibicao, tabelaDeTipos) {
        ...
        const cpInfo = {
            normal: calculateCPDetalhes(baseStats, minIVs, 20),
            perfect: calculateCPDetalhes(baseStats, { atk: 15, def: 15, hp: 15 }, 20)
        };
        const cpBoost = {
            normal: calculateCPDetalhes(baseStats, minIVs, 25),
            perfect: calculateCPDetalhes(baseStats, { atk: 15, def: 15, hp: 15 }, 25)
        };

        const typeIcons = validTipos.map(tipo => `<img src="${getTypeIconDetalhes(tipo)}" alt="${tipo}">`).join('');
        const weatherIcons = validTipos.map(tipo => `<img class="clima-boost" src="${getWeatherIconDetalhes(tipo)}">`).join('');

        const fraquezas = calcularFraquezasDetalhes(validTipos, tabelaDeTipos);
        const listaDeFraquezasHTML = Object.keys(fraquezas)
            .sort((a, b) => fraquezas[b] - fraquezas[a])
            .map(tipo => `
                <li class="detalhes-weakness-item">
                    <div class="detalhes-weakness-type">
                        <img src="${getTypeIconDetalhes(tipo)}" alt="${tipo}">
                        <span>${tipo}</span>
                    </div>
                    <span class="detalhes-weakness-percentage">${Math.round(fraquezas[tipo] * 100)}%</span>
                </li>
            `).join('');

        const fraquezasHTML = listaDeFraquezasHTML ? `<div class="detalhes-weakness-section"><h4>FRAQUEZAS</h4><ul class="detalhes-weakness-list">${listaDeFraquezasHTML}</ul></div>` : '';

        // Lógica de Detecção de Dinamax
        const isDynamax = /Dinamax/i.test(nomeExibicao);
        const shadowClass = isShadow ? 'is-shadow' : '';
        const dynamaxClass = isDynamax ? 'is-dynamax' : '';

        const pokemonImageHTML = `<div class="pokemon-image-container ${shadowClass} ${dynamaxClass}"><img class="img-detalhes" src="${pokemon.img}" alt="${nomeExibicao}"></div>`;

        return `<li class="ItemDetalhes ${validTipos.map(t => t.toLowerCase()).join(' ')}" style="background: ${gradientBackground};">
                    ${pokemonImageHTML}
    ...
    function alternarImagensDetalhes(pokemons, shinyPokemons) {
        const listas = document.querySelectorAll('.lista-detalhes li');
        listas.forEach(item => {
            const span = item.querySelector('span');
            if (!span) return;
            const nomeOriginal = span.textContent.trim();
            const img = item.querySelector('img.img-detalhes');

            if (img && nomeOriginal.includes('*')) {
                const nomeLimpo = limparNomePokemonDetalhes(nomeOriginal);
                let pokemon = pokemons.find(p => p.nome.toLowerCase() === nomeLimpo.toLowerCase());
                let shinyPokemon = shinyPokemons.find(s => s.nome.toLowerCase() === nomeLimpo.toLowerCase());
                
                if (!pokemon || !shinyPokemon) return;
    ...
    async function processListaDetalhes() {
        try {
            const { pokemons, shinyPokemons, dadosDosTipos } = await carregarDadosDetalhes();
            ...
            for (const pokemonListElement of pokemonLists) {
                ...
                const processedPokemonNames = pokemonNames.map(originalName => ({
                    ...
                    cleaned: limparNomePokemonDetalhes(originalName)
                }));

                const pokemonDataToRender = processedPokemonNames
                    .map(item => {
                        const pokemon = pokemons.find(p => p.nome.trim().toLowerCase() === item.cleaned.toLowerCase());
                        return pokemon ? { pokemon, nomeExibicao: item.original } : null;
                    })
                    .filter(Boolean);

                pokemonListElement.innerHTML = pokemonDataToRender.map(({ pokemon, nomeExibicao }) => {
                    const shinyPokemon = shinyPokemons.find(shiny => shiny.nome.toLowerCase() === pokemon.nome.toLowerCase());
                    return generatePokemonListItemDetalhes(pokemon, shinyPokemon, nomeExibicao, tabelaDeTipos);
                }).join('');
            }
            alternarImagensDetalhes(pokemons, shinyPokemons);
        } catch (error) {
            console.error('❌ Erro ao processar as listas de Detalhes de Pokémon:', error);
        }
    }

    processListaDetalhes();
