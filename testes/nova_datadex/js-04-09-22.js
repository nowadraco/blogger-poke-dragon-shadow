...
// =============================================================
window.showPokemonDetails = async function(baseSpeciesId, navigationList, targetSpeciesId) {
    window.scrollTo(0, 0);
...
function calcularMelhoresCombos(pokemon, oponenteInput, climaSelecionado = "Extreme") {
    ...
    fastMoves.forEach(fastId => {
        ...
        chargedMoves.forEach(chargedId => {
            ...
            // ✅ NOVO MOTOR DE SIMULAÇÃO DETERMINÍSTICA (LOOP REAL)
let energiaAtual = 0;
let danoTotal = 0;
let tempoAtual = 0;

// O relógio da batalha roda até o seu Pokémon desmaiar
while (tempoAtual < tempoDeVida) {
    if (energiaAtual >= enCost) {
        // Tenta soltar o Especial
        if (tempoAtual + tCharged <= tempoDeVida) {
            danoTotal += dmgCharged;
            tempoAtual += tCharged;
            energiaAtual -= enCost; // Gasta a energia e guarda a sobra!
        } else {
            // 💀 Morreu no meio do grito! Perdeu a energia e o dano não contou.
            tempoAtual = tempoDeVida; 
        }
    } else {
        // Precisa dar um Ataque Rápido para gerar mais energia
        if (tempoAtual + tFast <= tempoDeVida) {
            danoTotal += dmgFast;
            tempoAtual += tFast;
            energiaAtual += enGain;
            
            // Regra oficial da Niantic: Ninguém guarda mais de 100 de energia!
            if (energiaAtual > 100) energiaAtual = 100; 
        } else {
            // 💀 Morreu antes de terminar o tapa rápido.
            tempoAtual = tempoDeVida;
        }
    }
}
...
const tdo = danoTotal;
// Proteção contra divisão por zero, garantindo que o DPS seja realista
const dpsCombo = tempoDeVida > 0 ? (tdo / tempoDeVida) : 0; 
...
function calcularEstimador(meuPokemon, boss) {
    const tempoLimite = 300; // 300 segundos para Mega/Lendário
    const bossHPInicial = 9000; // Exemplo Mega
    
    // Simula a luta até o meu Pokémon morrer
    let resultadoSimulacao = simularAteMorrer(meuPokemon, boss); 
    // resultadoSimulacao retorna: {danoTotal, tempoSobrevivido}

    // Quantos Pokémon desses eu precisaria para tirar os 9000 de HP?
    let unidadesNecessarias = bossHPInicial / resultadoSimulacao.danoTotal;
    
    // Quanto tempo levaria no total (incluindo o tempo de reentrada/lobby)?
    let tempoTotal = unidadesNecessarias * resultadoSimulacao.tempoSobrevivido;
    let tempoDeReentrada = Math.floor(unidadesNecessarias / 6) * 15; // 15s para curar/voltar
    let timeToWin = tempoTotal + tempoDeReentrada;

    // O ESTIMADOR: timeToWin / tempoLimite da Raid
    let estimador = timeToWin / tempoLimite;

    return {
        estimador: estimador.toFixed(2), // Ex: 1.5 significa que precisa de 2 pessoas
        mortes: Math.ceil(unidadesNecessarias)
    };
}

// --- LÓGICA VISUAL DOS COUNTERS (AGORA ASSÍNCRONA COM LOADING) ---
window.atualizarListaCountersUI = async function(defensor, criterio) { // <-- Adicionado "async"
...
window.alternarCriterioCounters = function(novoCriterio) {
    ...
    document.getElementById('switch-dps').classList.toggle('active', novoCriterio === 'dps');
    document.getElementById('switch-tdo').classList.toggle('active', novoCriterio === 'tdo');
...
window.abrirRankingCompleto = async function(defensor) { // <-- Adicionado "async"
    window.scrollTo(0, 0);
    offsetCounters = 0;
